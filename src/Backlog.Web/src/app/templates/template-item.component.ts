...
const template = require("./template-item.component.html");
const styles = require("./template-item.component.scss");
...
export class TemplateItemComponent extends HTMLElement {
    ...
    private _onDeleteClick(e:Event) {
        this._templateService.remove({ id: this.entity.id }).then(() => {
            this.parentNode.removeChild(this);
        });
    }

    private _onEditClick() {
        this._router.navigate(["template", "edit", this.entity.id]);
    }

    private _onViewClick() {
        this._router.navigate(["template","view",this.entity.id]);
    }
